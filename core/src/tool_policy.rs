 ...
 * Adapted for the Ark agent tool access control model.
 */

//! Multi-layer tool policy resolution for Ark agents.
//!
...
//! This prevents subagents from accessing admin/scheduling tools and
//! limits deep spawn chains that could exhaust system resources.
...
pub struct ToolPolicyRule {
    /// Glob pattern to match tool names (e.g., "shell_*", "web_*").
    pub pattern: String,
...
pub fn resolve_tool_access(tool_name: &str, policy: &ToolPolicy, depth: u32) -> ToolAccessResult {
    ...
    // Phase 1: Check agent rules for denies (highest priority)
...
/// Tools denied to ALL subagents (depth > 0).
const SUBAGENT_DENY_ALWAYS: &[&str] = &[
    ...
    "hand_deactivate",
    "process_start",
    // Ark-specific: prevent subagents from accessing compiler/GCD
    "ark_compile",
    "gcd_invoke",
];

/// Tools denied to leaf subagents (depth >= max_depth - 1).
...
mod tests {
    ...
    #[test]
    fn test_ark_specific_deny() {
        let tools: Vec<String> = vec!["ark_compile", "gcd_invoke", "web_search"]
            .into_iter()
            .map(String::from)
            .collect();
        let filtered = filter_tools_by_depth(&tools, 1, 5);
