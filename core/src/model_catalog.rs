 ...
 * Adapted for the Ark model catalog — 130+ builtin models, 28 providers.
 */
...
//!
//! **Ark adaptation**: All types are inlined (no `openfang_types` dependency).
//! The `detect_auth` method removes the `crate::drivers::claude_code` reference.
    ...
    pub fn detect_auth(&mut self) {
        for provider in &mut self.providers {
            ...
            let has_fallback = match provider.id.as_str() {
                ...
                "claude-code" => false, // Ark: no CLI driver yet
                _ => false,
            };
