 ...
 * Adapted for the Ark agent approval gate system.
 */

//! Execution approval types for the Ark agent runtime.
//!
...
///
/// Uses a hex-encoded random string ID instead of UUID to avoid
/// adding the `uuid` crate as a dependency.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ApprovalRequest {
    /// Unique request identifier (hex-encoded random bytes).
    pub id: String,
    pub agent_id: String,
...
/// Generate a random hex-encoded request ID (16 bytes = 32 hex chars).
...
mod tests {
    ...
    #[test]
    fn request_timeout_boundaries_ok() {
        let mut req = valid_request();
        req.timeout_secs = 10;
        assert!(req.validate().is_ok());
