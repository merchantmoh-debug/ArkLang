 ...
 * Adapted for the Ark agent sandbox shell execution pipeline.
 */
...
//! the script file for env var patterns and returns warnings BEFORE execution,
//! adding a proactive defense layer to Ark's agent sandbox.
...
const SCRIPT_EXTENSIONS: &[&str] = &[
    ".py", ".sh", ".bash", ".rb", ".pl", ".js", ".ts", ".ps1", ".ark",
];
...
pub fn extract_env_var_refs(line: &str) -> Vec<String> {
...
pub fn format_warnings(warnings: &[ShellBleedWarning]) -> String {
    ...
    let mut output = String::from(
        "[ARK SECURITY — SHELL BLEED WARNING] The script references environment variables that may contain secrets:\n",
    );
    for w in warnings {
    ...
    output.push_str("Consider using tool parameters or Ark's credential vault instead.\n\n");
    output
}
...
mod tests {
    ...
    fn test_safe_vars_excluded() {
        ...
        assert!(SAFE_VARS.contains(&"ARK_HOME"));
    }
    ...
    fn test_extract_script_path() {
        ...
        assert_eq!(
            extract_script_path("ark run program.ark"),
            Some("program.ark".to_string())
        );
    }
    ...
    #[test]
    fn test_single_word_command_no_panic() {
        let warnings = scan_script_for_shell_bleed("ls", None);
        assert!(warnings.is_empty());
    }

    #[test]
    fn test_empty_command_no_panic() {
        assert_eq!(extract_script_path(""), None);
    }
}
